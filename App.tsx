...
import { collection, onSnapshot, doc, updateDoc, setDoc, getDocs, addDoc } from 'firebase/firestore';
...
const App: React.FC = () => {
  ...
  const checkAndSeedEvents = async () => {
    ...
    if (snapshot.empty) {
      const initialEvents = [
        // Day 1
        { date: '2026-01-30', location: '去程 (虎航IT602), 23:30到', category: EventCategory.TRANSPORT, notes: '' },
        { date: '2026-01-30', location: 'Momoho Hongdae', category: EventCategory.STAY, notes: '' },
        // Day 2
        { date: '2026-01-31', location: '弘大', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-01-31', location: 'Momoho Hongdae', category: EventCategory.STAY, notes: '' },
        // Day 3
        { date: '2026-02-01', location: '一日遊', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-01', location: 'Momoho Hongdae', category: EventCategory.STAY, notes: '' },
        // Day 4
        { date: '2026-02-02', location: '纛島玩雪橇 (10:00~17:00)', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-02', location: 'beton', category: EventCategory.FOOD, notes: '' },
        { date: '2026-02-02', location: 'ODE', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-02', location: '聖水', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-02', location: '(經紀公司)', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-02', location: '(星空圖書館)', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-02', location: 'Puradak炸雞', category: EventCategory.FOOD, notes: '' },
        { date: '2026-02-02', location: 'Momoho Hongdae', category: EventCategory.STAY, notes: '' },
        // Day 5
        { date: '2026-02-03', location: '廣藏市場', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-03', location: '明洞', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-03', location: 'Momoho Hongdae', category: EventCategory.STAY, notes: '' },
        // Day 6
        { date: '2026-02-04', location: '醫美', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-04', location: '東大門', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-04', location: '(溜冰)', category: EventCategory.SIGHTSEEING, notes: '' },
        { date: '2026-02-04', location: 'Momoho Hongdae', category: EventCategory.STAY, notes: '' },
        // Day 7
        { date: '2026-02-05', location: '回程 (大韓KE2027), 16:20飛機', category: EventCategory.TRANSPORT, notes: '' },
        { date: '2026-02-05', location: 'Momoho Hongdae', category: EventCategory.STAY, notes: '' },
      ];
              ...
              {members.map(member => (
                <div key={member.id} className="flex items-center gap-3 bg-slate-50 p-2 pr-4 rounded-2xl border border-slate-100">
                  <label className="relative cursor-pointer group shrink-0">
                    ...
                    <input type="file" accept="image/*" className="hidden" onChange={(e) => handleMemberAvatarChange(member.id, e)} />
                  </label>
                  <div className="flex-1">
                    <label className="text-[8px] font-bold text-slate-300 uppercase tracking-widest block ml-1 mb-0.5">旅伴姓名</label>
                    <input type="text" value={member.name} onChange={(e) => handleUpdateMemberName(member.id, e.target.value)} className="w-full bg-white px-3 py-1.5 rounded-xl text-sm font-bold text-slate-700 outline-none border border-slate-100 focus:border-sky-200 transition-colors" />
                  </div>
                </div>
              ))}
            </div>
            <button onClick={() => setIsSettingsOpen(false)} className="w-full py-4 bg-sky-400 text-white text-lg font-bold rounded-2xl shadow-active active:scale-95 transition-all">完成設定</button>
          </div>
        </div>
      )}
    </div>
